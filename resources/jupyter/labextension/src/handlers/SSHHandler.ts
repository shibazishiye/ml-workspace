...
export class SSHHandler {
   ...
   */
  static async getSetupCommand(originUrl: string): Promise<string> {
    const response = await fetch(
      this.baseUrl + 'tooling/ssh/setup-command?origin=' + encodeURIComponent(originUrl),
      {
        method: 'GET',
        credentials: 'include'
      }
   ...
   */
  static async getShareableToken(path: string): Promise<string> {
    const response = await fetch(
      this.baseUrl + 'tooling/token?path=' + encodeURIComponent(path),
      {
        method: 'GET',
        credentials: 'include'
      }
   ...
   */
  static async getFileLink(
    originUrl: string,
    path: string
  ): Promise<string> {
    const response = await fetch(
      this.baseUrl + 'tooling/files/link?origin=' + 
