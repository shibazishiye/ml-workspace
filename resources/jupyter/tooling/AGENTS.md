...
After editing Python files (like `jupyterTooling/routes.py`):

```bash
python -m py_compile jupyterTooling/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `jupyterTooling/*.py` files):
...
- Backend processing in `jupyterTooling/routes.py`
...
- **✅ Do**: `jupyterTooling/` with underscores, all lowercase
...
export namespace CommandIDs {
  export const openPanel = 'jupyterTooling:open-panel';
  export const refreshData = 'jupyterTooling:refresh-data';
}
...
- Backend routes: `jupyterTooling/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `jupyterTooling/` directory
...
1. **Read the backend first** — Check `jupyterTooling/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `jupyterTooling/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "jupyterTooling", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `jupyterTooling/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `jupyterTooling/`)
...
5. **Install the extension** (REQUIRED for JupyterLab to recognize it):
   ...
   jupyter server extension enable jupyterTooling
   ```
...
- **Output**: Creates files in `lib/` and `jupyterTooling/labextension/`
...
**After editing Python** (files in `jupyterTooling/`):
...
- Ask user to search for the extension ID (`jupyterTooling`) to see if it loaded
...
ls -la jupyterTooling/labextension/  # Should contain bundled extension
```
...
- Backend routes: `jupyterTooling/routes.py`
- Server extension config: `jupyter-config/server-config/jupyterTooling.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `jupyterTooling/routes.py`
...
**❌ Don't**: Use absolute imports like `from jupyterTooling.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'jupyterTooling:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyterTooling:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyterTooling-ClassName`
- **API routes** (in `jupyterTooling/routes.py`): `url_path_join(base_url, "jupyterTooling", "endpoint")`
